<#
Package a completed Super Mario RPG Windows release build.
...
without its SDL runtime and the recomp-ui assets/ next to it):
  ...
  SuperMarioRPGSNESRecomp-windows-x64-v<Version>.zip

This script does NOT build. Build first, e.g.:
  cmake --build build-release --target SuperMarioRPGSNESRecomp -j 4
...
# 0.1.0` to `0`, which is exactly how that happens in practice - always pass
...
$stageBase = 'SuperMarioRPGSNESRecomp'
...
# Unlike the other SNES recomps, Super Mario RPG has no repo-level config.ini:
# its presentation defaults live in the launcher settings struct and the
# launcher persists the player's choice next to the exe on first run. Ship one
# only if the repo grows one later.
$cfg = Join-Path $root 'config.ini'
if (Test-Path -LiteralPath $cfg) {
  (Get-Content $cfg) -replace '^Widescreen\s*=.*$', 'Widescreen = 0' |
