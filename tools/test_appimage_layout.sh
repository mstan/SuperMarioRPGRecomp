#!/bin/sh
# test_appimage_layout.sh <AppDir> â€” prove the packaged layout cannot trap
...
#   1. first launch seeds config.ini beside the (simulated) .AppImage â€” in a
#      writable dir, not the payload â€” plus any release-owned mod catalog;
...
# presentation defaults live in the launcher settings struct, so nothing ever
# writes one. The anchoring assertions therefore probe rom.cfg — the state file
# SMRPG genuinely does own — and the config.ini checks run only if a build
# starts producing one. Everything else is unconditional.
set -eu
...
# skip the GUI launcher. The game exits nonzero without a ROM; that is fine â€”
...
test -f "$state1/rom.cfg" || { echo "FAIL: rom.cfg not seeded beside the AppImage" >&2; exit 1; }
...
if [ -f "$state1/config.ini" ]; then
    has_config=1
else
    has_config=""
    echo "note: this build writes no config.ini; config assertions skipped"
fi

# 2. User state survives a relaunch: an edited state file and a user-installed
#    third-party mod package.
...
test -f "$state2/rom.cfg" || { echo "FAIL: moved AppImage did not re-anchor state" >&2; exit 1; }
if [ -n "$has_config" ]; then
...
#     the user with no route to Settings/Mods â€” and no way back, since the
#     launcher's file picker needs zenity/kdialog/yad to exist on the host.
state3=$tmp/state3
mkdir -p "$state3"
: > "$state3/pretend.sfc"
run_apprun "$state3/SuperMarioRPG.AppImage"
